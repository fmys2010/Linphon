...
这一步只安装 `linph` 命令。用 `curl` 直接运行时，脚本会先拉取仓库源码再构建。如果系统里还没有 Go，脚本会先自动安装发行版里的 Go 包；Debian/Ubuntu 上会使用 `apt-get install golang-go`，不是 `apt install go`。已经安装后再次运行同一条命令会先检查更新；没有更新时显示 `已是最新版本`，有更新时会替换本机的 `linph`。
...
交互式完整安装会询问是否定期重启来重新获取 IP，单位是小时，范围 `1-168`；也可以选择不启用。非交互安装默认不启用，需要时可用：

```bash
linph install --restart-every-hours 6
```
